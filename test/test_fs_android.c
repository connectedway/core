...
TEST_GROUP(fs_android);

TEST_SETUP(fs_android) {
    TEST_ASSERT_FALSE_MESSAGE(test_startup(), "Failed to Startup Framework");
}

TEST_TEAR_DOWN(fs_android) {
    test_shutdown();
}

TEST(fs_android, test_fs_android) {
    OFC_INT ret;
    ofc_thread_create_local_storage();
    ret = test_file(OFC_TEST_FS_ANDROID_PATH);
    ofc_thread_destroy_local_storage();
    TEST_ASSERT_FALSE_MESSAGE(ret, "File Test Failed");
}

TEST_GROUP_RUNNER(fs_android) {
    RUN_TEST_CASE(fs_android, test_fs_android);
}
